...
内层轮询（~30s）和外层查询之间存在最大 30s 的状态漂移窗口。若 Pangen 在此窗口内异常退出，Claude Code 获取到的状态可能仍为 `running`。此 limitations 在第一版中已知，暂无主动修复计划，Claude Code 应结合 `job_logs` 做辅助判断。
...
| 场景 | 处理方式 |
|------|---------|
| SSH 连接失败 | 返回 error，不重试，Claude Code 决策是否重试 |
